...
export class FirewallService {
   ...
   */
  async addToSet(setName: 'blacklist' | 'whitelist', ip: string): Promise<void> {
    this.validateIp(ip);
    ...
    try {
      ...
      this.logger.error(`Failed to add IP ${ip} to ${setName}: ${error.message}`);
   ...
   * @throws InternalServerErrorException if exec command fails
   */
  async removeFromSet(setName: 'blacklist' | 'whitelist', ip: string): Promise<void> {
    this.validateIp(ip);
