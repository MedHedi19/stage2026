...
export class ThreatIntelligenceGenerator implements ReportGenerator {
  ...
  private processThreatIntelligence(alerts: any[], countryStats: { countryCode: string; count: number }[]) {
    ...
    return {
      summary: {
        ...
        uniqueIOCs: iocs.uniqueIPs.size + iocs.uniqueDomains.size + iocs.uniqueHashes.size,
  ...
  private filterSecurityAlerts(alerts: any[]) {
    ...
    const securityGroups = ['ids', 'suricata', 'authentication_failed', 'attack', 'malware', 'vulnerability', 'web', 'sql'];
  ...
  private extractIOCs(alerts: any[]) {
    ...
    const ipReputation: Record<string, { count: number; severity: number; lastSeen: string }> = {};
    const domainReputation: Record<string, { count: number; severity: number; lastSeen: string }> = {};
    ...
    alerts.forEach(alert => {
      ...
      if (alert.data?.src_ip) {
        ...
        if (!ipReputation[ip]) {
          ipReputation[ip] = { count: 0, severity: 0, lastSeen: alert.timestamp };
        }
        ipReputation[ip].count++;
        ipReputation[ip].severity = Math.max(ipReputation[ip].severity, alert.rule.level);
        ...
        domains.forEach(domain => {
          if (domain.includes('.')) {
            ...
            if (!domainReputation[domain]) {
              domainReputation[domain] = { count: 0, severity: 0, lastSeen: alert.timestamp };
            }
            domainReputation[domain].count++;
            domainReputation[domain].severity = Math.max(domainReputation[domain].severity, alert.rule.level);
          }
        });
      }
    ...
    return {
      ...
      uniqueHashes,
      uniqueURLs,
      ipReputation: Object.entries(ipReputation).map(([ip, data]) => ({ ip, ...data, risk: data.severity >= 8 ? 'High' : data.severity >= 5 ? 'Medium' : 'Low' })),
      domainReputation: Object.entries(domainReputation).map(([domain, data]) => ({ domain, ...data, risk: data.severity >= 8 ? 'High' : data.severity >= 5 ? 'Medium' : 'Low' })),
      highRiskIPs,
      mediumRiskIPs,
    };
  }
  ...
  private analyzeMalwareFamilies(alerts: any[]) {
    const families: Record<string, { count: number; severity: number; descriptions: string[] }> = {};
    
    const malwareIndicators = [
      'emotet', 'trickbot', 'ryuk', 'maze', 'conti', 'lockbit', 'wannacry', 'notpetya',
      'cobalt strike', 'mimikatz', 'powershell empire', 'metasploit', 'shellcode',
      'trojan', 'backdoor', 'rat', 'botnet', 'ransomware', 'cryptominer', 'stealer',
      'loader', 'dropper', 'downloader', 'injector', 'rootkit', 'bootkit'
    ];
    ...
    alerts.forEach(alert => {
      ...
      malwareIndicators.forEach(indicator => {
        if (description.includes(indicator) || groups.some(g => g.toLowerCase().includes(indicator))) {
      ...
      if (!detectedFamily) {
        ...
        } else if (description.includes('trojan') || description.includes('backdoor')) {
          detectedFamily = 'trojan';
        } else if (description.includes('bot') || description.includes('ddos')) {
          detectedFamily = 'botnet';
        } else if (description.includes('miner') || description.includes('crypto')) {
          detectedFamily = 'cryptominer';
        } else if (description.includes('steal') || description.includes('credential')) {
          detectedFamily = 'stealer';
        } else if (description.includes('inject') || description.includes('hook')) {
      ...
      if (detectedFamily) {
        if (!families[detectedFamily]) {
          families[detectedFamily] = { count: 0, severity: 0, descriptions: [] };
        }
        families[detectedFamily].count++;
        families[detectedFamily].severity = Math.max(families[detectedFamily].severity, alert.rule.level);
        if (!families[detectedFamily].descriptions.includes(alert.rule.description)) {
    ...
    const topFamilies = Object.entries(families)
      .sort((a, b) => (b[1].count as number) - (a[1].count as number))
  ...
  private attributeThreatActors(alerts: any[]) {
    const actors: Record<string, { count: number; techniques: string[]; confidence: string }> = {};
    
    // Threat actor TTPs (Tactics, Techniques, Procedures)
    const threatActorProfiles = {
      'APT29 (Cozy Bear)': {
        indicators: ['spear phishing', 'password spraying', 'web shell', 'dll injection'],
        techniques: ['T1566', 'T1110', 'T1505', 'T1055'],
        confidence: 'Medium'
      },
      'APT28 (Fancy Bear)': {
        indicators: ['brute force', 'credential dumping', 'remote access', 'exploit'],
        techniques: ['T1110', 'T1003', 'T1021', 'T1190'],
        confidence: 'High'
      },
      'Lazarus Group': {
        indicators: ['ransomware', 'cryptocurrency', 'supply chain', 'living off the land'],
    ...
    alerts.forEach(alert => {
      ...
      Object.entries(threatActorProfiles).forEach(([actor, profile]) => {
        ...
        if (matchCount >= 2) {
          if (!actors[actor]) {
            actors[actor] = { count: 0, techniques: profile.techniques, confidence: profile.confidence };
          }
          actors[actor].count++;
        }
      });
  ...
  private analyzeAttackPatterns(alerts: any[]) {
    const patterns: Record<string, { count: number; severity: number; examples: string[] }> = {};
    
    const attackPatterns = {
      'Command & Control': ['c2', 'command and control', 'beacon', 'callback', 'tunnel'],
      'Lateral Movement': ['lateral', 'pass the hash', 'pass the ticket', 'remote service', 'smb'],
      'Exfiltration': ['exfil', 'data transfer', 'upload', 'ftp', 'dns tunnel'],
      'Persistence': ['persistence', 'scheduled task', 'registry', 'service', 'startup'],
      'Defense Evasion': ['defense evasion', 'obfuscation', 'encoding', 'encryption', 'anti-debug'],
      'Credential Access': ['credential', 'password', 'hash dump', 'mimikatz', 'kerberos'],
    ...
    alerts.forEach(alert => {
      ...
      Object.entries(attackPatterns).forEach(([pattern, indicators]) => {
        if (indicators.some(indicator => description.includes(indicator))) {
          ...
          patterns[pattern].severity = Math.max(patterns[pattern].severity, alert.rule.level);
  ...
  private analyzeCampaigns(alerts: any[]) {
    const campaigns: Record<string, { alerts: any[]; sourceIPs: Set<string>; timeRange: { start: string; end: string } }> = {};
    ...
    alerts.forEach(alert => {
      ...
      if (new Date(alert.timestamp) < new Date(campaigns[campaignKey].timeRange.start)) {
        campaigns[campaignKey].timeRange.start = alert.timestamp;
      }
      if (new Date(alert.timestamp) > new Date(campaigns[campaignKey].timeRange.end)) {
    ...
    const activeCampaigns = Object.entries(campaigns)
      ...
      .map(([key, data]) => ({
        ...
        duration: this.calculateDuration(data.timeRange.start, data.timeRange.end),
  ...
  private async generatePdfBuffer(data: any, filters: any): Promise<Buffer> {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        doc.fontSize(22).fillColor('#0b192c').text('Threat Intelligence Report', { align: 'center' });
        doc.moveDown();
        ...
        // Executive Summary
        doc.fontSize(16).fillColor('#0b192c').text('Threat Landscape Summary', { underline: true });
        doc.moveDown();
        ...
        doc.fontSize(12).fillColor('#0b192c').text(`Unique IPs: ${data.iocs.uniqueIPs.size}`);
        doc.fontSize(12).fillColor('#0b192c').text(`Unique Domains: ${data.iocs.uniqueDomains.size}`);
        doc.fontSize(12).fillColor('#0b192c').text(`Unique File Hashes: ${data.iocs.uniqueHashes.size}`);
        doc.moveDown();

        doc.fontSize(14).fillColor('#0b192c').text('High-Risk IP Addresses');
        doc.moveDown();

        data.iocs.highRiskIPs.slice(0, 10).forEach(({ ip, count, severity, risk }) => {
          doc.fontSize(10).fillColor('black').text(`${ip} - ${count} alerts (Severity: ${severity}, Risk: ${risk})`);
          doc.moveDown(0.3);
        });
        ...
        // Geolocation Analysis
        doc.fontSize(16).fillColor('#0b192c').text('Geographic Threat Distribution', { underline: true });
        doc.moveDown();
        ...
        data.geoAnalysis.topCountries.forEach(({ country, count }) => {
          const threatLevel = count > 50 ? 'Critical' : count > 20 ? 'High' : count > 10 ? 'Medium' : 'Low';
          const threatColor = threatLevel === 'Critical' ? '#ef4444' : threatLevel === 'High' ? '#f59e0b' : threatLevel === 'Medium' ? '#3b82f6' : '#10b981';
        ...
        // Malware Analysis
        doc.fontSize(16).fillColor('#0b192c').text('Malware Family Analysis', { underline: true });
        doc.moveDown();

        data.malwareAnalysis.topFamilies.slice(0, 8).forEach(({ family, count, severity }) => {
          doc.fontSize(11).fillColor('#0b192c').text(`${family.toUpperCase()}`);
          doc.fontSize(10).fillColor('#64748b').text(`Incidents: ${count} | Max Severity: ${severity}`);
          doc.moveDown(0.5);
        });
        ...
        // Threat Actor Attribution
        doc.fontSize(16).fillColor('#0b192c').text('Threat Actor Attribution', { underline: true });
        doc.moveDown();

        if (data.threatActorAnalysis.topActors.length > 0) {
          data.threatActorAnalysis.topActors.forEach(({ actor, count, techniques, confidence }) => {
            doc.fontSize(12).fillColor('#0b192c').text(actor);
            doc.fontSize(10).fillColor('#64748b').text(`Attribution Count: ${count} | Confidence: ${confidence}`);
            doc.text(`MITRE Techniques: ${techniques.join(', ')}`);
            doc.moveDown(0.8);
          });
        } else {
          doc.fontSize(10).fillColor('#64748b').text('No specific threat actors identified with high confidence.');
          doc.moveDown();
        }
        ...
        // Attack Patterns
        doc.fontSize(16).fillColor('#0b192c').text('Attack Pattern Analysis', { underline: true });
        doc.moveDown();

        data.attackPatterns.topPatterns.forEach(({ pattern, count, severity, examples }) => {
          doc.fontSize(11).fillColor('#0b192c').text(pattern);
          doc.fontSize(10).fillColor('#64748b').text(`Occurrences: ${count} | Severity: ${severity}`);
        ...
        // Campaign Analysis
        doc.fontSize(16).fillColor('#0b192c').text('Active Campaigns', { underline: true });
        doc.moveDown();

        data.campaignAnalysis.activeCampaigns.forEach((campaign, index) => {
          doc.fontSize(12).fillColor('#0b192c').text(`Campaign ${index + 1}: ${campaign.name}`);
          doc.fontSize(10).fillColor('#64748b').text(`Alerts: ${campaign.alertCount} | Duration: ${campaign.duration} | Severity: ${campaign.severity}`);
          doc.text(`Source IPs: ${campaign.sourceIPs.slice(0, 5).join(', ')}${campaign.sourceIPs.length > 5 ? '...' : ''}`);
          doc.moveDown(0.8);
        });

        // Emerging Threats
        doc.fontSize(16).fillColor('#0b192c').text('Emerging Threats (Last 7 Days)', { underline: true });
        doc.moveDown();

        data.emergingThreats.emergingThreats.forEach(({ threat, count, trend }) => {
          doc.fontSize(10).fillColor('black').text(`${threat}: ${count} incidents (${trend})`);
          doc.moveDown(0.3);
        });
  ...
  private async generateExcelBuffer(data: any, filters: any): Promise<Buffer> {
    ...
    const summaryData = [
      ...
      ['Total Security Alerts', data.summary.totalAlerts.toLocaleString(), 'Active', '→'],
      ['Unique IOCs Detected', data.summary.uniqueIOCs.toLocaleString(), 'Monitored', '↑'],
      ['Countries Affected', data.summary.countriesAffected.toString(), 'Global', '→'],
      ['Malware Families', data.summary.malwareFamilies.toString(), 'Identified', '→'],
    ...
    worksheet.getCell(`C${row}`).value = data.iocs.highRiskIPs.length > 0 ? 'High' : 'Medium';
    row++;
    ...
    if (data.threatActorAnalysis.topActors.length > 0) {
      ...
      worksheet.mergeCells(`A${row}:E${row}`);
      worksheet.getCell(`A${row}`).value = 'No specific threat actors identified with high confidence';
      row++;
    }
