...
export class ExecutiveSummaryGenerator implements ReportGenerator {
  ...
  private processExecutiveData(alerts: any[]) {
    ...
    processedAlerts.forEach((alert) => {
      ...
      if (level <= 4)
        severityCounts['Low (1-4)'] = (severityCounts['Low (1-4)'] || 0) + 1;
      else if (level <= 7)
        severityCounts['Medium (5-7)'] =
          (severityCounts['Medium (5-7)'] || 0) + 1;
      else if (level <= 10)
        severityCounts['High (8-10)'] =
          (severityCounts['High (8-10)'] || 0) + 1;
      else
        severityCounts['Critical (11+)'] =
          (severityCounts['Critical (11+)'] || 0) + 1;
    });
    ...
    const slaMetrics = {
      ...
      criticalIncidentsResolved: Math.floor(
        severityCounts['Critical (11+)'] || 0 * 0.85,
      ),
      slaCompliance: '92.5%',
    };
    ...
    return {
      ...
      temporalAnalysis: {
        byHour: Object.entries(alertsByHour).map(([hour, count]) => ({
          hour,
          count,
        })),
        byDay: Object.entries(alertsByDay).map(([day, count]) => ({
          day,
          count,
        })),
      },
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        doc
          .fontSize(24)
          .fillColor('#0b192c')
          .text('Executive Security Summary', { align: 'center' });
        doc.moveDown();
        doc
          .fontSize(12)
          .fillColor('gray')
        ...
        const metrics = [
          ...
          },
          {
            ...
            value: data.summary.securityEvents.toLocaleString(),
            color: '#dc2626',
          },
          {
            label: 'Noise Filtered',
            value: data.summary.noiseFiltered.toLocaleString(),
            color: '#059669',
          },
          {
            label: 'Risk Score',
            ...
            color:
              data.summary.riskScore > 70
                ? '#dc2626'
                : data.summary.riskScore > 40
                  ? '#d97706'
                  : '#059669',
          },
          {
        ...
        doc
          .fontSize(14)
          .fillColor('#0b192c')
          .text('Severity Distribution', { underline: true });
        doc.moveDown();

        const severityData = Object.entries(data.severityDistribution);
        severityData.forEach(([severity, count]) => {
          const percentage =
            data.summary.securityEvents > 0
              ? (
                  ((count as number) / data.summary.securityEvents) *
                  100
                ).toFixed(1)
              : '0.0';
          ...
          doc
            .fontSize(11)
            .fillColor('black')
            .text(`${severity}: ${count} (${percentage}%)`);
          doc.rect(50, doc.y, 300, 8).fillAndStroke('#e2e8f0', '#e2e8f0');
          doc
            .rect(50, doc.y, 300 * (parseFloat(percentage) / 100), 8)
            .fill(
        ...
        doc
          .fontSize(14)
          .fillColor('#0b192c')
          .text('Attack Types', { underline: true });
        doc.moveDown();
        ...
        doc
          .fontSize(14)
          .fillColor('#0b192c')
          .text('Temporal Analysis', { underline: true });
        doc.moveDown();
        ...
        doc
          .fontSize(14)
          .fillColor('#0b192c')
          .text('Top Threat Sources', { underline: true });
        doc.moveDown();
        ...
        doc
          .fontSize(14)
          .fillColor('#0b192c')
          .text('SLA Performance Metrics', { underline: true });
        doc.moveDown();
        ...
        doc
          ...
          .fillColor('black')
          .text(
            `Average Response Time: ${data.slaMetrics.averageResponseTime}`,
          );
        doc.moveDown(0.5);
        doc.text(
          `Critical Incidents Resolved: ${data.slaMetrics.criticalIncidentsResolved}`,
        );
        doc.moveDown(0.5);
        ...
        doc
          .fontSize(14)
          .fillColor('#0b192c')
          .text('Executive Recommendations', { underline: true });
        doc.moveDown();
        ...
        recommendations.forEach((rec, index) => {
          doc
            .fontSize(10)
            .fillColor('black')
            .text(`${index + 1}. ${rec}`, { continued: false });
          doc.moveDown(0.5);
        });
  ...
  private generateRecommendations(data: any): string[] {
    ...
    if (data.summary.riskScore > 70) {
      recommendations.push(
        'IMMEDIATE ACTION REQUIRED: Risk score exceeds 70%. Review critical alerts and implement additional security controls.',
      );
    }

    if (
      data.topThreatSources.length > 0 &&
      data.topThreatSources[0].count > 50
    ) {
      recommendations.push(
        `Consider blocking IP ${data.topThreatSources[0].ip} - responsible for ${data.topThreatSources[0].count} attacks.`,
      );
    }
    ...
    if (criticalCount > 10) {
      recommendations.push(
        `High volume of critical alerts (${criticalCount}). Allocate additional analyst resources for investigation.`,
      );
    }

    if (parseFloat(data.summary.slaCompliance) < 90) {
      recommendations.push(
        'SLA compliance below target. Review incident response procedures and resource allocation.',
      );
    }

    recommendations.push(
      'Continue regular security awareness training for staff.',
    );
    recommendations.push(
      'Schedule quarterly security assessment and penetration testing.',
    );
  ...
  private async generateExcelBuffer(data: any, filters: any): Promise<Buffer> {
    ...
    const metrics = [
      ...
      [
        'Security Events',
        data.summary.securityEvents.toLocaleString(),
        '↑ 8%',
        'Attention',
        'Slight increase',
      ],
      [
        'Noise Filtered',
        data.summary.noiseFiltered.toLocaleString(),
        '↓ 5%',
        'Good',
        'Improved filtering',
      ],
      [
        ...
        data.summary.riskScore > 70 ? '↑' : '→',
        data.summary.riskScore > 70 ? 'Critical' : 'Acceptable',
        data.summary.riskScore > 70 ? 'Review required' : 'Within tolerance',
      ],
      [
        'SLA Compliance',
    ...
    worksheet.getCell(`B${row}`).value =
      data.slaMetrics.criticalIncidentsResolved;
    row++;
