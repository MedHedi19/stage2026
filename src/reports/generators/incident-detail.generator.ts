...
interface ProcessedIncident extends Omit<
  Incident,
  'sourceIPs' | 'destIPs' | 'affectedSystems'
> {
  sourceIPs: string[];
  destIPs: string[];
...
export class IncidentDetailGenerator implements ReportGenerator {
  ...
  private processIncidentData(alerts: any[]) {
    ...
    const highIncidents = incidents.filter(
      (i) => i.maxSeverity >= 8 && i.maxSeverity < 11,
    ).length;
    ...
    return {
      summary: {
        ...
        mediumIncidents: incidents.filter(
          (i) => i.maxSeverity >= 5 && i.maxSeverity < 8,
        ).length,
  ...
  private analyzeResponseTimes(incidents: ProcessedIncident[]) {
    ...
    const avgDuration =
      responseTimes.reduce((sum, rt) => sum + rt.duration, 0) /
    ...
    return {
      ...
      responseTimeDistribution: responseTimes.map((rt) => ({
        ...
        duration:
          rt.durationMinutes >= 60
            ? `${Math.floor(rt.durationMinutes / 60)}h ${rt.durationMinutes % 60}m`
            : `${rt.durationMinutes}m`,
      })),
    };
  }
  ...
  private analyzeIncidentLifecycle(incidents: ProcessedIncident[]) {
    const lifecycle = {
      detection: incidents.filter(
        (i) => i.status === 'Critical' || i.status === 'High',
      ).length,
      analysis: incidents.filter((i) => i.status === 'Medium').length,
      containment: Math.floor(
        incidents.filter((i) => i.actions.includes('IP Blocked')).length,
      ),
      eradication: Math.floor(
        incidents.filter((i) => i.actions.includes('File Quarantined')).length,
      ),
      recovery:
        incidents.length -
        Math.floor(
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        doc
          .fontSize(22)
          .fillColor('#0b192c')
          .text('Incident Detail Report', { align: 'center' });
        doc.moveDown();
        doc
          .fontSize(12)
          .fillColor('gray')
        ...
        doc
          .fontSize(16)
          .fillColor('#0b192c')
          .text('Incident Summary', { underline: true });
        doc.moveDown();
        ...
        doc
          .fontSize(16)
          .fillColor('#0b192c')
          .text('MITRE ATT&CK Tactics Coverage', { underline: true });
        doc.moveDown();
        ...
        Object.entries(data.mitreMapping).forEach(([tactic, techniques]) => {
          ...
          if (techniquesArray.length > 0) {
            doc
              .fontSize(12)
              .fillColor('#0b192c')
              .text(`${tactic}: ${techniquesArray.length} techniques`);
            doc
              .fontSize(9)
              .fillColor('#64748b')
              .text(techniquesArray.join(', '));
            doc.moveDown(0.5);
          }
        });
        ...
        doc
          .fontSize(16)
          .fillColor('#0b192c')
          .text('Response Time Analysis', { underline: true });
        doc.moveDown();
        ...
        doc
          ...
          .fillColor('black')
          .text(
            `Average Response Time: ${data.responseTimeAnalysis.averageResponseTime}`,
          );
        doc.moveDown(0.5);
        doc.text(
          `Maximum Response Time: ${data.responseTimeAnalysis.maxResponseTime}`,
        );
        doc.moveDown(0.5);
        doc.text(
          `Minimum Response Time: ${data.responseTimeAnalysis.minResponseTime}`,
        );
        doc.moveDown();
        ...
        doc
          .fontSize(16)
          .fillColor('#0b192c')
          .text('Incident Lifecycle Analysis', { underline: true });
        doc.moveDown();
        ...
        doc
          .fontSize(16)
          .fillColor('#0b192c')
          .text('Top Affected Assets', { underline: true });
        doc.moveDown();
        ...
        doc
          .fontSize(16)
          .fillColor('#0b192c')
          .text('Detailed Incident Analysis', { underline: true });
        doc.moveDown();

        const criticalIncidents = data.incidents
          .filter((i) => i.maxSeverity >= 8)
          .slice(0, 10);
        ...
        criticalIncidents.forEach((incident, index) => {
          ...
          doc
            .fontSize(12)
            .fillColor('#0b192c')
            .text(`Incident #${incident.id}: ${incident.description}`);
          doc.moveDown(0.3);
          ...
          doc
            ...
            .fillColor('#64748b')
            .text(
              `Severity: ${incident.status} | Alerts: ${incident.alertCount}`,
            );
          doc.text(
            `Time Range: ${new Date(incident.firstSeen).toLocaleString()} - ${new Date(incident.lastSeen).toLocaleString()}`,
          );
          doc.text(
            `Affected Systems: ${incident.affectedSystems.join(', ') || 'N/A'}`,
          );
  ...
  private async generateExcelBuffer(data: any, filters: any): Promise<Buffer> {
    ...
    worksheet.getCell(`B${row}`).value =
      data.responseTimeAnalysis.averageResponseTime;
    row++;

    worksheet.getCell(`A${row}`).value = 'Maximum Response Time';
    worksheet.getCell(`B${row}`).value =
      data.responseTimeAnalysis.maxResponseTime;
    row++;

    worksheet.getCell(`A${row}`).value = 'Minimum Response Time';
    worksheet.getCell(`B${row}`).value =
      data.responseTimeAnalysis.minResponseTime;
    row++;
    ...
    data.incidents.forEach((incident) => {
      ...
      incidentSheet.getCell(incidentRow, 6).value = new Date(
        incident.firstSeen,
      ).toLocaleString();
      incidentSheet.getCell(incidentRow, 7).value = new Date(
        incident.lastSeen,
      ).toLocaleString();
      incidentSheet.getCell(incidentRow, 8).value =
        incident.sourceIPs.join(', ');
      incidentSheet.getCell(incidentRow, 9).value =
        incident.mitreTechniques.join(', ');
      incidentSheet.getCell(incidentRow, 10).value =
