...
export class FirewallListTrafficGenerator implements ReportGenerator {
  ...
  constructor(
    ...
    private readonly blacklistRepository: Repository<BlacklistEntry>,
    @InjectRepository(WhitelistEntry)
    private readonly whitelistRepository: Repository<WhitelistEntry>,
  ) {}

  getReportTypeName() {
    return 'Firewall List Traffic';
  }

  async generate({
    format,
    filters,
  }: ReportGeneratorData): Promise<GeneratedReport> {
    const [blacklist, whitelist] = await Promise.all([
      this.blacklistRepository.find({
        where: { active: true },
    ...
    let alerts: any[] = [];
    try {
      alerts = await this.wazuhService.fetchRecentAlerts({
      ...
      this.logger.warn(
        `Unable to retrieve Wazuh traffic data; exporting list entries with zero observed traffic: ${error.message}`,
      );
    }
    ...
    const buffer =
      format === 'excel'
        ? await this.excel(rows, filters)
        : await this.pdf(rows, filters);
    return {
      buffer,
      filename: `firewall-list-traffic-${stamp}.${format === 'excel' ? 'xlsx' : 'pdf'}`,
    };
  }
  ...
  private buildRows(
    ...
    const mapEntry = (entry: any, list: ListRow['list']): ListRow => ({
      ...
      events: stats.get(entry.ip)?.events || 0,
      bytes: stats.get(entry.ip)?.bytes || 0,
    });
    return [
      ...blacklist.map((entry) => mapEntry(entry, 'Blacklist')),
      ...whitelist.map((entry) => mapEntry(entry, 'Whitelist')),
  ...
  private extractBytes(alert: any): number {
    const values = [
      alert?.data?.bytes,
      alert?.data?.src_bytes,
      alert?.data?.dest_bytes,
      alert?.data?.flow?.bytes,
      alert?.data?.network?.bytes,
    ];
  ...
  private async pdf(rows: ListRow[], filters: any): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      ...
      doc
        ...
        .fillColor('#64748b')
        .text(
          `Period: ${filters.startDate || 'All available data'} - ${filters.endDate || 'Now'}`,
          { align: 'center' },
        );
      doc.moveDown(1.5);
      for (const list of ['Blacklist', 'Whitelist'] as const) {
        ...
        doc
          .fontSize(15)
          .fillColor(list === 'Blacklist' ? '#b91c1c' : '#047857')
        ...
        if (!items.length) {
          doc
            .fontSize(10)
            .fillColor('#64748b')
            .text(`No ${list.toLowerCase()} entries.`);
          doc.moveDown();
          continue;
        }
        ...
        for (const row of items) {
          ...
          doc
            .fillColor('#0f172a')
            .fontSize(8.5)
            .text(row.ip, 45, y, { width: 95 });
          doc
            .fillColor('#475569')
            .text(row.reason, 142, y, { width: 125, ellipsis: true });
          doc.fillColor('#0f172a').text(row.events.toLocaleString(), 270, y, {
            width: 55,
            align: 'right',
          });
          doc.text(this.formatBytes(row.bytes), 332, y, {
            width: 75,
            align: 'right',
          });
          doc
            .fillColor('#475569')
            .text(new Date(row.addedAt).toLocaleDateString(), 414, y, {
              width: 65,
            });
          y += 22;
        }
      ...
      doc
        ...
        .fillColor('#64748b')
        .text(
          'Traffic is calculated from byte fields present in Wazuh events. “No byte data” means the source event did not include byte counters.',
        );
      doc.end();
    });
  }
  ...
  private tableHeader(doc: any) {
    const y = doc.y;
    doc.rect(42, y, 510, 19).fill('#e2e8f0');
    ...
    [
      ...
      ['Traffic', 332],
      ['Added', 414],
    ].forEach(([text, x]) => doc.text(text, x, y + 6));
    doc.y = y + 25;
  }

  private async excel(rows: ListRow[], filters: any): Promise<Buffer> {
    const workbook = new Workbook();
    const sheet = workbook.addWorksheet('Firewall List Traffic');
    sheet.columns = [
      { width: 14 },
      { width: 18 },
      { width: 30 },
    ...
    sheet.getCell('A1').value = 'Firewall List Traffic Report';
    ...
    sheet.addRow([
      `Period: ${filters.startDate || 'All data'} - ${filters.endDate || 'Now'}`,
    ]);
    sheet.addRow([]);
